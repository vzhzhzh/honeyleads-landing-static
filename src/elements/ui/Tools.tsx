...
const Tools: FC<ToolsProps> = ({}) => {
  return (
    <Container className="mt-70 pb-150 xs:pb-50">
      <h2 className="inline text-50 leading-60 font-semibold text-purple xs:text-22 xs:leading-26">Инструменты <div className='inline text-[#28D2AE]'>HoneyLeads</div></h2>
      <div className='flex flex-nowrap gap-x-50 mt-75 md:overflow-x-scroll md:pb-30 xs:gap-x-20 xs:mt-35'>
        {cards.map(({ title, description, img}, index) => <div key={index} className="min-w-[30%] rounded-42 shadow-CARD px-20 py-50 bg-white md:min-w-[26.875rem] xs:min-w-[14rem] xs:py-15 xs:rounded-22">
